...
@section('banner')
    <x-banner>
        ...
        <x-slot name='subtitle'>
            Rellena el formulario para actualizar la liga {{$league->name}}
        </x-slot>
    </x-banner>
@endsection
    ...
    <section class="bg-[#F4F7FF] py-14 lg:py-20 text-gray-600 body-font relative">
        <div class="container mx-auto">
            <div class="mt-10 md:mt-0 md:col-span-2 shadow bg-white overflow-hidden sm:rounded-md">
                <form  method="POST" action="{{ route('league.update' , $league) }}" class="px-10">
                    @csrf
                    @method('put')
                    <div class="">
                            ...
                            <div id="LeagueInfo">
                                    ...
                                    <div x-data="{ photoName: null, photoPreview: null }"
                                        ...
                                        <div class="text-center flex flex-col justify-center items-center">
                                            ...
                                            <div class="mt-2 " x-show="! photoPreview">
                                                @if (str_contains($league->logo, 'https://'))
                                                <img src="{{ $league->logo }}" alt="image"
                                                class="w-20 h-20 rounded-full shadow" />
                                            @else
                                                <img src="{{ Vite::asset('\public\storage\images\entities/' . $league->logo) }}" alt="image"
                                                class="w-20 h-20 rounded-full shadow" />
                                            @endif
                                ...
                                <div class="grid grid-cols-6 gap-6 mb-2">
                                    <x-forms.input id='name' type='text' placeholder='3X3 LLEFIÀ'
                                        class='sm:row-span-1' value='{{old("name", $league->name)}}'>
                                        Nombre
                                        @error('name')
                                            <small class="text-primary">*{{ $message }}</small>
                                        @enderror
                                    </x-forms.input>
                                    <x-forms.input id='min_age' type='number' placeholder='1' min='0'
                                        class='sm:row-span-1' value='{{old("min_age", $league->min_age)}}'>
                                ...
                                <div class="px-4 col-span-6 sm:col-span-3 mt-12">
                                    <h4 class="block text-sm font-medium text-gray-700">Basket Court</h4>
                                    <hr class="mb-4">
                                </div>
                                <div class="px-4 grid grid-cols-6 gap-6 mb-2">
                                    <x-forms.input id='street' type='text'
                                        placeholder='Avinguda del Doctor Bassols' class='sm:row-span-1' value='{{old("street", $basket_court->street)}}'>
                                        Calle
                                        @error('street')
                                        <small class="text-primary">*{{ $message }}</small>
                                    @enderror
                                    </x-forms.input>
                                    <x-forms.input id='number' type='number' placeholder='10' value='{{old("number", $basket_court->number)}}'>
                                        Número
                                        @error('number')
                                        <small class="text-primary">*{{ $message }}</small>
                                    @enderror
                                    ...
                                    <x-forms.input id='city' type='text' placeholder='Badalona' value='{{old("city", $basket_court->city)}}'>
                                        Ciudad
                                        @error('city')
                                        <small class="text-primary">*{{ $message }}</small>
                                    @enderror
                    ...
                    <div id="saveInfo" class="flex gap-2 justify-center p-6 bg-white text-center sm:px-6">
                        {{-- <button action="{{ route('league.show', compact('league')) }}"
                    ...
                    <button  type="submit" class="inline-flex items-center justify-center rounded bg-primary py-3 px-12 text-base font-medium text-white transition duration-300 ease-in-out hover:bg-dark">
                        Guardar
                    </button>
                    </div> 
                </form>
